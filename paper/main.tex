...
\begin{enumerate}
    \item A \textbf{decomposed signal-reliability gate} that combines per-frame landmark-stability, brightness, and cue-consistency sub-scores into a continuous reliability index, and multiplicatively attenuates the fused fatigue evidence \emph{before} temporal accumulation --- while never suppressing severe-fatigue alarms.
    \item A \textbf{variance-based speech-jitter filter} that suppresses mouth-motion (talking) false positives using the temporal variance of the mouth aspect ratio.
    \item A \textbf{2D/3D metric separation} that computes the mouth aspect ratio in the image plane to avoid monocular depth inflation, reported as a supporting engineering result.
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Facial Landmark \& Geometry Processing}: MediaPipe FaceMesh landmarks; EAR from 3D eye geometry and MAR restricted to the 2D image plane to avoid monocular depth inflation.
    ...
    \item \textbf{Reliability-Gated Fusion}: multiplicative attenuation of fused fatigue evidence before temporal accumulation; severe states exempt from suppression.
    ...
    \item \textbf{Speech-Jitter Filter}: MAR-variance gating to suppress talking false positives.
\end{itemize}
...
operating-point metric cannot observe. Motivated by this analysis, event-level
evaluation (false alarms per hour and per-episode latency) is the immediate next
experiment.
...
effect. Future work is therefore an event-level evaluation (false alarms per
hour and per-episode latency), on-device Raspberry~Pi~4 profiling, and on-road
validation.
